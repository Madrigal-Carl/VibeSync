...
async function predictRandomMood() {
    try {
        ...
        const data = await res.json();
        console.log("Random prediction result:", data);

        const features = data.features; // show randomized features
        const percentage = data.confidence;
        const mood = data.mood;
        const recommendedSongs = data.recommended_songs || [];

        // Update input fields to show randomized features
        for (const key in features) {
            const elem = document.getElementById(key);
            if (elem) elem.value = features[key];
        }
        ...
        console.error("Error predicting random mood:", err);
        alert("Error predicting random mood. Check console.");
    }
}
